...
class PSO:
    ...
    def __init__(self, n_networks):
        ...
        self.networks = [NN.NeuralNetwork(NN.x,NN.y, self.neurons) for i in range(self.n_networks)]
        self.global_best_value = float("inf")
        self.global_best_position = NN.NeuralNetwork(NN.x,NN.y, self.neurons).getParams.shape
...
if __name__ == "__main__":
    ...
    while(iterations < n_iterations):
        print(f"Iteration: {iterations} GlobalBestVal: {pso.global_best_value}") #(check if iteration work)
        pso.optimise() 
